...
function removeFirstOccurrences(str, value) {
  ...
  const indexStart = oldStr.indexOf(value);
  const indexEnd = indexStart + valueLength;
  const newStr =
    indexStart >= 0
      ? oldStr.slice(0, indexStart).concat(oldStr.slice(indexEnd))
      : oldStr;
  return newStr;
}
...
function removeLastOccurrences(str, value) {
  ...
  const oldStr = str;
  const valueLength = value.length;
  const indexStart = oldStr.lastIndexOf(value);
  const indexEnd = indexStart + valueLength;
  const newStr =
    indexStart >= 0
      ? oldStr.slice(0, indexStart).concat(oldStr.slice(indexEnd))
...
function countVowels(str) {
  const chars = 'a, e, i, o, u, y, A, E, I, O, U, Y';
...
function isPalindrome(str) {
  const strToLower = str.toLocaleLowerCase().replace(/[^A-Za-z0–9]/g, '');
  const reversedStr = strToLower.split('').reverse().join('');
  return strToLower === reversedStr;
}
...
function encodeToRot13(str) {
  ...
  const encodedChars = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';

  let encodedStr = '';
  for (let i = 0; i < str.length; i += 1) {
    if (chars.includes(str[i])) {
      const charIndex = chars.indexOf(str[i]);
      encodedStr = encodedStr.concat(encodedChars[charIndex]);
    } else {
      encodedStr = encodedStr.concat(str[i]);
    }
  }
  return encodedStr;
}
